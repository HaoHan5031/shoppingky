...
- (BOOL) isTableOK
{
    ...
    if (!openSuccess) {
        ...
        NSLog(@"地址数据库打开成功");
        FMResultSet *rs = [self.fmdb executeQuery:@"SELECT count(*) as 'count' FROM sqlite_master WHERE type ='table' and name = locationTabble"];
        while ([rs next])
        {
...
- (void)requestGetData{
    ...
    NSMutableArray* arr = [self queryAllProvince];
    if (arr.count > 0) {
        return;
    }
        ...
        @try
        {
            for (AddressItem * item in self.dataArray) {
                ...
                NSString *insertSql= [NSString stringWithFormat:
                                      @"INSERT INTO locationTabble ('code','sheng','di','xian','name', 'level') VALUES ('%@','%@','%@','%@','%@','%@')",
                                      item.code, item.sheng,item.di,item.xian ,item.name, item.level];
